...
if (isset($_GET['id'])) {
    ...
    // Fetch order details with company name and customer name
    $query = "SELECT o.*, c.name as company_name, cust.name as customer_name 
              FROM Orders o
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['confirm_delete'])) {
    ...
    $status_check = "SELECT status FROM Orders WHERE id = ?";
    $check_stmt = mysqli_prepare($conn, $status_check);
    mysqli_stmt_bind_param($check_stmt, "i", $id);
    mysqli_stmt_execute($check_stmt);
    $check_result = mysqli_stmt_get_result($check_stmt);
    $order_status = mysqli_fetch_assoc($check_result)['status'];

    if ($order_status === 'delivered') {
        $error = 'Cannot delete order as it has already been delivered.';
    } else {
        // Delete the order
        ...
        if (mysqli_stmt_execute($stmt)) {
             $log_message = "Order #$id deleted by user #" . $_SESSION['user_id'];
                            ...
                            <div class="ml-3">
                                ...
                                <div class="mt-2 text-sm text-red-700">
                                    <?php if ($order['status'] === 'delivered'): ?>
                                        <p>This order cannot be deleted because it has already been delivered.</p>
                                    <?php else: ?>
                    ...
                    <form action="" method="POST" class="space-y-4">
                        ...
                        <div class="space-y-2">
                            ...
                            <ul class="text-sm text-gray-600">
                                ...
                                <li><strong>Delivery Date:</strong> <?php echo $order['delivery_date'] ? date('M d, Y', strtotime($order['delivery_date'])) : 'Not specified'; ?></li>
                            </ul>
                        </div>
