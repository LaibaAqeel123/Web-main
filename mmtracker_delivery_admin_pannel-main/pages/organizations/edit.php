...
// Add validation functions (Copied from pages/orders/edit.php)
function validateUKAddressWithMapbox($address_line1, $city, $postal_code, $country, $address_line2 = '') {
    ...
    $address_query = urlencode("$postal_code, $full_address, $city, $country"); // Renamed variable
...
$org_query = "SELECT o.* FROM Organizations o
              WHERE o.id = ?";
...
// Initialize form data with organization values
$formData = [
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $formData['phone'] = '+44' . preg_replace('/[^0-9]/', '', trim($_POST['phone'] ?? ''));
    ...
    if (empty($formData['name'])) {
    ...
    } elseif (empty($formData['address_line1'])) {
        $error = 'Address Line 1 is required';
    } elseif (empty($formData['city'])) {
        $error = 'City is required';
    } elseif (empty($formData['postal_code'])) {
        $error = 'Postal Code is required';
    } elseif (!empty($formData['phone']) && !validateUKPhoneNumber($formData['phone'])) {
        $error = 'Please enter a valid UK phone number';
    } elseif (!isValidUKPostcode($formData['postal_code'])) {
        $error = 'Please enter a valid UK postcode';
    } elseif (!validateUKAddressWithMapbox(
        ...
        // Check if another organization with same name already exists for this company
        $check_query = "SELECT id FROM Organizations WHERE name = ? AND company_id = ? AND id != ?";
        $check_stmt = mysqli_prepare($conn, $check_query);
        mysqli_stmt_bind_param($check_stmt, "sii", $formData['name'], $formData['company_id'], $organization_id);
        mysqli_stmt_execute($check_stmt);
        $check_result = mysqli_stmt_get_result($check_stmt);
        
        if (mysqli_num_rows($check_result) > 0) {
            $error = 'Another organization with this name already exists for the selected company';
        } else {
            // Process logo upload if provided
            $logo_url = $organization['logo_url']; // Keep existing logo by default
            $logo_url = null; // Force logo_url to null
                ...
                if (!in_array($file['type'], $allowed_types)) {
                    ...
                    if (empty($error)) {
                        ...
                        if (move_uploaded_file($file['tmp_name'], $upload_path)) {
                            // If we have an old logo, remove it
                            if ($organization['logo_url'] && file_exists('../../' . $organization['logo_url'])) {
                                unlink('../../' . $organization['logo_url']);
                            }
            ...
            */
            ...
            // If an old logo exists (even if not used), ensure it's removed from filesystem
            if ($organization['logo_url'] && file_exists('../../' . $organization['logo_url'])) {
                unlink('../../' . $organization['logo_url']);
            }
            ...
            if (empty($error)) {
                ...
                if (mysqli_stmt_execute($update_stmt)) {
                    $formData['logo_url'] = $logo_url; // Update form data with new logo URL (will be null)
                    $success = 'Organization updated successfully';
                } else {
                    $error = 'Failed to update organization: ' . mysqli_stmt_error($update_stmt);
                    
                    // If there was an error and we uploaded a new file, delete it (Should not happen now)
                    /*
                    if ($logo_url !== $organization['logo_url'] && $logo_url && file_exists('../../' . $logo_url)) {
                        unlink('../../' . $logo_url);
                    }
                    */
                }
...
<head>
    ...
    <title>Edit Organization - <?php echo SITE_NAME; ?></title>
