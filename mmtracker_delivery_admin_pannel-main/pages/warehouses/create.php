...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$error) {
        $query = "INSERT INTO Warehouses (name, address, city, state, postal_code, country, 
                                        latitude, longitude, company_id, status) 
                  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'active')";
        
        $stmt = mysqli_prepare($conn, $query);
        mysqli_stmt_bind_param($stmt, "ssssssddi", 
            $formData['name'], $formData['address'], $formData['city'], 
            $formData['state'], $formData['postal_code'], $formData['country'], 
            $formData['latitude'], $formData['longitude'], $formData['company_id']
        );
        ...
        if (mysqli_stmt_execute($stmt)) {
            ...
            $error = 'Error creating warehouse: ' . mysqli_error($conn);
        }
...
<head>
    ...
    <title>Add Warehouse - <?php echo SITE_NAME; ?></title>
                ...
                <div class="bg-white shadow-md rounded-lg overflow-hidden p-6">
                    <form action="" method="POST" class="space-y-6">
                        ...
                        <div class="flex justify-end space-x-3">
                            ...
                            <button type="submit" class="bg-indigo-600 text-white px-4 py-2 rounded-md hover:bg-indigo-700">Create Warehouse</button>
                        </div>
                    </form>
                </div>
            </div>
